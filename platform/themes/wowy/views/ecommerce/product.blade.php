...
@php
    ...
    include('C:\Users\Komal Aggarwal\OneDrive\Desktop\NEWW\platform\themes\wowy\views\ecommerce\simple_html_dom.php');


    function GetFlipkartPrice($product){
    $fp = file_get_html($product->Flipkart_Link);
    $Flipkart_Price = $fp->find('div[id=container] div[class=_30jeq3 _16Jk6d]',0)->plaintext;
            ...
            <div class="detail-info">
                ...
                <div class="clearfix product-price-cover">
                    <div class="product-price primary-color float-left">
                        ...
                        @endif
                        <br>
                        @if($product->Flipkart_Link!=NULL)
                            <button class="button button-buy-now @if ($product->isOutOfStock()) btn-disabled @endif" type="submit" name="checkout" @if ($product->isOutOfStock()) disabled @endif>{{ __('Flipkart  ') }}{{GetFlipkartPrice($product)}}</button>
                            <button class="button button-buy-now @if ($product->isOutOfStock()) btn-disabled @endif" type="submit" name="checkout" @if ($product->isOutOfStock()) disabled @endif>{{ __('Amazon') }}</button>
